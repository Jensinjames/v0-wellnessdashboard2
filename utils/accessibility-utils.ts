...
/**
 * Creates an ID provider hook for a component
 *
 * @param prefix - The prefix to use for IDs
 * @returns A function that generates unique IDs with the given prefix
 */
export function createIdProvider(prefix: string) {
  return function useComponentId(suffix?: string): string {
    const id = React.useMemo(() => {
      const baseId = suffix ? `${prefix}-${suffix}` : prefix
      return generateUniqueId(baseId)
    }, [suffix])
