/**
 * Supabase Server Client for Pages Router
 * This version doesn't use next/headers and is compatible with the Pages Router
 */
...
export function createServerSupabaseClientForPages(
  req: NextApiRequest,
  res: NextApiResponse,
  options: {
  ...
  return createServerClient<Database>(supabaseUrl, supabaseKey, {
    cookies: {
      ...
      set(name: string, value: string, options: CookieOptions) {
        res.setHeader("Set-Cookie", `${name}=${value}; Path=${options.path || "/"}; Max-Age=${options.maxAge || 3600}`)
      },
