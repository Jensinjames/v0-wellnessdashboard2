...
import { TOKEN_EVENTS, initTokenRefreshMonitoring } from "@/lib/token-manager"
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    const cleanup = initTokenRefreshMonitoring()

    // Listen for token refresh events
    const handleTokenRefreshSuccess = () => {
      authLogger.debug("Token refreshed successfully")
      // Refresh the session data
      authService.getSession().then(({ session: refreshedSession }) => {
        if (refreshedSession) {
    ...
    const handleTokenRefreshFailure = (event: CustomEvent) => {
      authLogger.error("Token refresh failed:", event.detail.error)
      // Show a toast notification if in debug mode
      if (isDebugMode()) {
        toast({
          title: "Authentication Warning",
          description: "Failed to refresh your session. You may need to sign in again.",
          variant: "destructive",
        })
      }
    }
    ...
    // Add event listeners
    window.addEventListener(TOKEN_EVENTS.REFRESH_SUCCESS, handleTokenRefreshSuccess as EventListener)
    window.addEventListener(TOKEN_EVENTS.REFRESH_FAILURE, handleTokenRefreshFailure as EventListener)
    window.addEventListener(TOKEN_EVENTS.SESSION_EXPIRED, handleSessionExpired as EventListener)

    return () => {
      // Remove event listeners
      window.removeEventListener(TOKEN_EVENTS.REFRESH_SUCCESS, handleTokenRefreshSuccess as EventListener)
      window.removeEventListener(TOKEN_EVENTS.REFRESH_FAILURE, handleTokenRefreshFailure as EventListener)
      window.removeEventListener(TOKEN_EVENTS.SESSION_EXPIRED, handleSessionExpired as EventListener)
      cleanup()
    }
  }, [router, toast])
  ...
  const resendVerificationEmail = async (email: string): Promise<{ success: boolean; error: string | null }> => {
    try {
      ...
      const { success, error } = await authService.resendVerificationEmail(sanitizedEmail)
