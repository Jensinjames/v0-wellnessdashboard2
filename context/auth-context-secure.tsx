...
import { getSupabaseClient, resetSupabaseClient } from "@/lib/supabase-client-consolidated"
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateProfile = useCallback(
    async (data: ProfileFormData): Promise<{ success: boolean; error: Error | null }> => {
      if (!user) {
  ...
  const updatePassword = useCallback(async (password: string): Promise<{ success: boolean; error: string | null }> => {
    try {
      const supabaseClient = await getSupabaseClient()
  ...
  const getClientInfo = useCallback(() => {
    return {
      hasClient: !!supabaseClientRef.current,
