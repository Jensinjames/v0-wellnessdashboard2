...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    if (routeType === ROUTE_TYPES.PROTECTED) {
      ...
      // Only for specific routes that require verified email
      const routesRequiringVerification = ["/profile/complete", "/secure-dashboard"]
      if (!isEmailVerified && routesRequiringVerification.some((route) => pathname.startsWith(route))) {
