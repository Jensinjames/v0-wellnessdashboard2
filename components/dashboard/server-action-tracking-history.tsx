...
interface ServerActionTrackingHistoryProps {
  entries: WellnessEntry[]
  categories: WellnessCategory[]
}

export function ServerActionTrackingHistory({ entries, categories }: ServerActionTrackingHistoryProps) {
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [entryToDelete, setEntryToDelete] = useState<WellnessEntry | null>(null)

  // Set up optimistic action for delete
  const { execute: executeDeleteEntry, isLoading: isDeleting } = useOptimisticAction({
    action: (entryId: string) => deleteEntry(entryId),
    table: "wellness_entries",
    operation: "delete",
    getOptimisticData: (entryId) => ({ id: entryId, __deleted: true }),
    onSuccess: () => {
  ...
  const optimisticEntries = optimistic.applyUpdates<WellnessEntry>("wellness_entries", entries)

  // Filter out deleted entries
  const filteredEntries = optimisticEntries.filter((entry) => !entry.__deleted)
  ...
  const confirmDelete = async () => {
    ...
    await executeDeleteEntry(entryToDelete.id)
  }
