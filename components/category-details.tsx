...
import { MetricsErrorFallback } from "@/components/metrics-error-fallback"
...
export function CategoryDetails() {
  ...
  const getCategoryData = (categoryId: string, period: ComparisonPeriod = "current") => {
    ...
    if (category.metricsLoadedSuccessfully === false) {
      return []
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          {enabledCategories.slice(0, 4).map((category) => (
            <TabsContent key={category.id} value={category.id} className="p-4">
              {category.metricsLoadedSuccessfully === false ? (
                <MetricsErrorFallback
                  categoryId={category.id}
                  categoryName={category.name}
                  onRetry={() => refreshData()}
                  error={`Failed to load metrics for ${category.name}`}
                />
              ) : (
