...
import { safeCn, conditionalCn, colorCn } from "@/utils/safe-class-names"
...
export function EnhancedEntriesTable({
  ...
  // Get category color class for progress bars - using the new colorCn utility
  const getCategoryProgressColor = (category: WellnessCategory): string => {
    return colorCn("bg", category.color, "500")
  }
                    ...
                    return (
                      <>
                        <TableRow key={entryId} className="group">
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <Badge className={safeCn(getScoreBadgeColor(overallScore), "shadow-sm text-white")}>
                                {overallScore}%
                              </Badge>
                              <Progress
                                value={overallScore}
                            ...
                            return (
                              <TableCell key={category.id} className="hidden md:table-cell">
                                <div className="flex items-center gap-2">
                                  <Badge
                                    variant="outline"
                                    className={safeCn(getCategoryColorClass(category.id), "shadow-sm")}
                                  >
                        ...
                        <TableRow
                          ...
                          className={conditionalCn("", { hidden: !isExpanded })}
                                  ...
                                  {enabledCategories.map((category) => (
                                    <div key={category.id} className="space-y-2">
                                      <h4 className="text-sm font-medium flex items-center gap-2">
                                        <div
                                          className={colorCn("bg", category.color, "500") + " w-3 h-3 rounded-sm"}
                                        ></div>
                                        {category.name}
                                      </h4>
                  ...
                  return (
                    <div key={entry.id} className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {goalData.map((item) => {
                          // Create a string class name for the progress indicator using colorCn
                          const progressClass = colorCn("bg", item.colorName, "500")
